...
const NavItem = memo(function NavItem({
  ...
  if (isMobile) {
    return (
      <Link
        href={item.href}
        className="relative flex flex-col items-center justify-center flex-shrink-0 w-[68px] py-2.5"
      >
        <div
          className={cn(
            "absolute inset-x-0.5 inset-y-0.5 rounded-xl transition-all duration-200",
            isActive ? "bg-primary/15 border border-primary/20" : "bg-transparent"
          )}
        />
        <item.icon
          className={cn(
  ...
  return (
    <Link
      href={item.href}
      className="relative w-16 h-12 flex items-center justify-center rounded-xl group"
    >
      <div
        className={cn(
          "absolute inset-0 rounded-xl transition-all duration-200",
          isActive ? "bg-primary/15 border border-primary/20" : "bg-transparent group-hover:bg-primary/10"
        )}
      />
      <item.icon
        className={cn(
...
export const Navigation = memo(function Navigation() {
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex fixed left-0 top-0 bottom-0 w-24 flex-col items-center py-7 glass border-r border-border/50 z-40 overflow-y-auto scrollbar-hide">
        <Link href="/" className="mb-8 flex-shrink-0">
          <div className="w-11 h-11 rounded-full bg-primary/12 border border-primary/20 flex items-center justify-center transition-all hover:scale-105 hover:bg-primary/18 active:scale-95">
